...
@utils.register_model(name='ncsnpp_multihead')
class NCSNppMultiHead(nn.Module):
    """NCSN++ model"""
    ...
    def __init__(self, config, num_classes=10):
        ...
        if progressive != 'output_skip':
            modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                        num_channels=in_ch, eps=1e-6))
            modules.append(conv3x3(in_ch, channels * num_classes, init_scale=init_scale))

        self.all_modules = nn.ModuleList(modules)
        if num_classes > 0:
            self.label_embed = nn.Embedding(num_classes, nf * 4)
    ...
    def forward(self, x, time_cond, y=None):
        ...
        if self.conditional:
            ...
            temb = None
        if y is not None:
            class_embed = self.label_embed(y)
