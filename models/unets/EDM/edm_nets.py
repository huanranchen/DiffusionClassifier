...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        if class_labels is not None:
            labels_tensor = torch.zeros((class_labels.shape[0], self.label_dim), device=x.device)
            labels_tensor[torch.arange(class_labels.shape[0]), class_labels.to(torch.int)] = 1
